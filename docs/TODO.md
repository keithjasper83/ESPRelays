...
| COR-05 | P1 | Correctness | Open | Define deterministic retry/backoff behavior spec for Wi-Fi outages. | `src/WiFiManager.cpp` |
...
| MEM-01 | P1 | Memory/Perf | Open | Audit heavy `String` concatenation paths for heap-fragmentation risk. | `src/WebControlServer.cpp`, `src/ScheduleManager.cpp` |
...
| OBS-01 | P1 | Observability | Open | Add structured event codes for connectivity, schedule dispatch, and OTA attempts. | `src/main.cpp`, `src/WiFiManager.cpp`, `src/OtaUpdateManager.cpp` |
...
MQTT-specific tasks SEC-05, MEM-03 and DEP-01 were retired with removal of the MQTT experiment in 4.2.0.
